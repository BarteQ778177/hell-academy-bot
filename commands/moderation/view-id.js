...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('view-id')
        .setDescription('View user moderation via ID')
    ...
    async execute(interaction) {
        ...
        if (/^\d+$/.test(targetInput)) {
            target = await interaction.client.users.fetch(targetInput);
        } else if (/^<@!?(\d+)>$/.test(targetInput)) {
            const idMatch = /^<@!?(\d+)>$/.exec(targetInput);
            if (idMatch) {
                target = await interaction.client.users.fetch(idMatch[1]);
            }
        }
        
        await interaction.deferReply({ ephemeral: true });

        if (!target) return await interaction.editReply({ content: 'Invalid user or user ID.', ephemeral: true });
        ...
        await interaction.editReply({ embeds: [embed], components: [first], ephemeral: true });
